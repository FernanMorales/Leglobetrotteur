        ...
        if(intRegex.test(e))
        {
            ...
            if (result.length != 0)
            {
                ...
                else
                {
                    ...
                    var script = "<script>$('#comune').selectbox().next().css({'width': '199px', 'left': '169px'})"+"\;"+"</script>";
                    $("#aggiunta").html("<select id='comune' name='comune'><option value=''>Choisissez une ville</option>"+city_select+"</select>"+script).show("fast");
                }
            }
            else
