<?php
	...
	function curl_execute($data){
		$url = "http://www.myoutlets.it/maisenza/iscrizione_concorso_turista.php";
	...
	/* ACCESSO CONNESSIONE DB INTERNO - DISATTIVATO */
	/*define("connectDB", true);
	include("connect.php");*/
	/*$table = array (
		"users" => "turistacurioso_users",
	...
	$method = array("INSERT" => "POST", "UPDATE" => "PUT", "SELECT" => "GET", "DELETE" => "DELETE"); 
	// POST -> INSERT ****** 
	// PUT -> UPDATE ***** 
	// GET -> SELECT ***** 
	// DELETE -> DELETE ***** 
	...
	switch($_POST['position']){
		...
		case 'c':
			...
			// PIXEL RICHIESTO DA ALAN IN DATA 09/04/2014 per tracciamento partners
			
				//$pixel = file_get_contents("https://tbl.tradedoubler.com/report?organization=1850173&event=290032&leadNumber=".$id[1]);			
			...
			try{
				...
				if($check['response'] == "ok"){
					//se esiste il contatto recupero "listId" e "conId" e preparo l'array per la cancellazione
					$delete_temp = array(
						"reason" => "temporary delete before reinsert",
						"listId" => $check['data']['listId'],
						"conId" => $check['data']['conId'],
						"email" => $_POST['email']
					);
				
					//cancello
					$delete_md = new OAuthRequester($url, $method['DELETE'], $delete_temp);
					$result_delete = $delete_md->doRequest();
				}

				// reinserimento completo su mdirector
